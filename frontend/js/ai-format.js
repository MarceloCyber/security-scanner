(function () {
    ...
    function ensureStyles() {
        ...
        style.textContent = `
            .ai-rich-answer{max-width:min(92%,780px)!important;white-space:normal!important;line-height:1.65!important}
    ...
    function render(markdown) {
        ...
        for (let index = 0; index < lines.length; index += 1) {
            ...
            closeList();
            const sourceClass = /^fonte\s*:/i.test(line) ? ' class="ai-source"' : '';
            output.push(`<p${sourceClass}>${inline(line)}</p>`);
        }
        closeList();
        return output.join('');
    }
